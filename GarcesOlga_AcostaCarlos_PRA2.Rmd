...
**CARLOS: HAY QUE ACTUALIZAR LAS CONCLUSIONES (VARIANZAS IGUALES O NO) UNA VEZ CONFIRMADO EL JUEGO DE DATOS**

*El valor p-value que obtenemos es de 0.02. Este valor es menor que alfa e indica que con un nivel de confianza del 95% podemos rechazar la hipótesis nula de que las varianzas son iguales. Las varianzas en la edad de los pasajeros que sobrevivieron y los que no sobrevivieron son diferentes.*

```{r echo=TRUE, message=FALSE, warning=FALSE}
var.test(train.survived$Age, train.not.survived$Age)
```

**CARLOS: CREO QUE DEBERIAMOS HACER LO DE LA VARIANZA PARA SIBSP, PARCH, COUNT.TICKET Y PRICE.UNIT PQ SON REALMENTE NUMERICAS**

**CARLOS:NO SE PQ NO PUEDO HACERLO CON PRICE.UNIT**

```{r echo=TRUE, message=FALSE, warning=FALSE}
#var.test(train.survived$Price.unit, train.not.survived$Price.unit)
```
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
plot(train.young$Survived)
plot(train.older$Survived)
```
...
tc<-table( train.first.second$Pclass, train.first.second$Survived )
tc
...
Hipótesis nula - H0: La proporción de hombres que sobreviven es igual o mayor a 50% -> H0: p >= p0, siendo po = 0.5  
...
**CARLOS: CHI CUADRADO ES PARA VER SI SON INDEPENDIENTES O NO, VERDAD? COMO LO RELACIONAMOS PARA COMPROBAR "EN MAYOR MEDIDA"?**
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
t.test(train.not.survived$Age, train.survived$Age, alternative="greater", var.equal=FALSE)
```
...
model1=glm(formula=Survived~Sex+Pclass, data=train, family=binomial(link=logit))
summary(model1)
```

Calculamos los odds ratio. Igual que en el caso anterior vemos que la probabilidad ser hombre y sobrevivir es mucho menor que la de ser mujer y sobrevivir. La probabilidad de sobrevivir en segunda clase es **(VARIA RESULTADO)** 0.43 de la probabilidad de sobrevivir en primera clase. Para la tercera clase la probabilidad de sobrevivir es incluso menor.  
```{r echo=TRUE, message=FALSE, warning=FALSE}
...
model2=glm(formula=Survived~factor(Count.ticket)+Embarked, data=train, family=binomial(link=logit))
summary(model2)
...
**CARLOS: CREO QUE ESTO LO PODEMOS BORRAR PQ YA TENEMOS EL ARCHIVO TEST PREPARADO Y LIMPIO)**

**HAY QUE MIRAR QUE MÉTODOS, SUPERVISADOS O NO SUPERVISADOS QUEREMOS HACER**


```{r echo=TRUE, message=FALSE, warning=FALSE}
#set.seed(1)
#train.random <- train[sample(nrow(train)),]
```
